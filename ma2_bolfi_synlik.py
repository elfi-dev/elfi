...
def MA2(t1, t2, n_obs=100, batch_size=n_sims, random_state=None):
...
def standard_bsl(ssy, ssx):
    print('ssx', ssx[0], 'ssy', ssy[0])
    ssx = ssx[0]
    ssy = ssy[0]
    print('ssx', ssx.shape, 'ssy', ssy.shape)
    # TODO: robust for bolfi?
    ssx = np.transpose(ssx)
    ssy = np.transpose(ssy)
...
Y = elfi.Simulator(MA2, m['t1'], m['t2'], observed=y_obs, name="MA2")
def dummy_func(x):
...
d = elfi.Distance(standard_bsl, S1, y_obs)
...
result_BOLFI = bolfi.sample(2000)
